...
// Retiran: vista independiente basada en Logistica pero filtrada por tipo_transporte 'retira'
function Retiran() {
  ...
  useEffect(() => {
    ...
    if (filterCliente) {
      filtered = filtered.filter(item => item.cliente?.toLowerCase().includes(filterCliente.toLowerCase()));
    }
    ...
    if (filterCompletado) {
      if (filterCompletado === 'completado') filtered = filtered.filter(item => item.completado);
  ...
  const handleCompleted = async (item) => {
    try {
      ...
      const newCompletedState = !item.completado;
      if (newCompletedState) await api.put(`${endpoint}/${item.id}/completado`);
      else await api.put(`${endpoint}/${item.id}`, { completado: false });
      setCombinedData(prev => prev.map(row => row.id === item.id && row.tipo === item.tipo ? { ...row, completado: newCompletedState } : row));
    } catch (error) {
  ...
  const handleExportExcel = () => {
    ...
    const wb = XLSX.utils.book_new(); XLSX.utils.book_append_sheet(wb, ws, 'Retiran');
    const fecha = new Date().toISOString().slice(0,10);
    XLSX.writeFile(wb, `Retiran_${fecha}.xlsx`);
  };
    ...
    <Box sx={{ p: 3 }}>
      <Box sx={{ display: 'flex', gap: 2, mb: 2, flexWrap: 'wrap', alignItems: 'center' }}>
        <Box sx={{ display: 'flex', gap: 2, flexWrap: 'wrap', alignItems: 'center' }}>
          <TextField label="Filtrar por Vendedor" value={filterVendedor} onChange={e => setFilterVendedor(e.target.value)} size="small" sx={{ minWidth: 140 }} />
          <TextField label="Filtrar por Cliente" value={filterCliente} onChange={e => setFilterCliente(e.target.value)} size="small" sx={{ minWidth: 140 }} />
          <TextField label="Filtrar por Armador" value={filterArmador} onChange={e => setFilterArmador(e.target.value)} size="small" sx={{ minWidth: 140 }} />
          <TextField label="Filtrar por Tipo Tte" value={filterTipoTte} onChange={e => setFilterTipoTte(e.target.value)} size="small" sx={{ minWidth: 140 }} />
          <TextField label="Filtrar por Transporte" value={filterTransporte} onChange={e => setFilterTransporte(e.target.value)} size="small" sx={{ minWidth: 140 }} />
        </Box>

        <FormControl size="small" sx={{ minWidth: 120 }}>
          <Select value={filterEstado} onChange={(e) => setFilterEstado(e.target.value)} displayEmpty>
            <MenuItem value="">Todos los Estados</MenuItem>
            {Array.isArray(estados) && estados.map((estado) => (<MenuItem key={estado.id} value={estado.id}>{estado.nombre}</MenuItem>))}
          </Select>
        </FormControl>
        <FormControl size="small" sx={{ minWidth: 140 }}>
          <Select value={filterCompletado} onChange={(e) => setFilterCompletado(e.target.value)} displayEmpty>
      ...
      <Box sx={{ display: 'flex', gap: 2, mb: 2, alignItems: 'center' }}>
        <Button variant="contained" startIcon={<FileDownload />} onClick={handleExportClick} sx={{ fontWeight: 600, px: 2.5, py: 1.2, borderRadius: 2 }}>Exportar</Button>
        ...
        <Box sx={{ display: 'flex', gap: 1, ml: 2 }}>
          <Typography variant="body2" sx={{ backgroundColor: '#e3f2fd', padding: '4px 12px', borderRadius: '16px', color: '#1976d2', fontSize: '0.75rem' }}>Pendientes: {filteredData.filter(item => !item.completado).length}</Typography>
          <Typography variant="body2" sx={{ backgroundColor: '#f3e5f5', padding: '4px 12px', borderRadius: '16px', color: '#7b1fa2', fontSize: '0.75rem' }}>Completados: {filteredData.filter(item => item.completado).length}</Typography>
        </Box>
      </Box>
        ...
        <Table size="small" sx={{ '& .MuiTableCell-root': { fontSize: '0.75rem', padding: '6px 8px' } }}>
          <TableHead>
            <TableRow sx={{ backgroundColor: '#f5f5f5' }}>
              {columns.map((column) => (<TableCell key={column.id} style={{ minWidth: column.minWidth, fontWeight: 'bold', fontSize: '0.8rem' }}>{column.label}</TableCell>))}
            </TableRow>
          </TableHead>
          <TableBody>
            {filteredData.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).map((row, index) => {
              const isCompleted = row.completado;
              const rowStyle = isCompleted ? { backgroundColor: '#f5f5f5', opacity: 0.6, '&:hover': { backgroundColor: '#eeeeee' } } : { '&:hover': { backgroundColor: '#f9f9f9' } };
              return (
      ...
      <Dialog open={editModalOpen} onClose={handleCloseEdit}>
        ...
        <DialogContent>
          <FormControl fullWidth margin="normal">
            <Select value={editingTipoTransporte} onChange={(e) => setEditingTipoTransporte(e.target.value)} displayEmpty>
              <MenuItem value="">Seleccionar Tipo de Transporte</MenuItem>
              {Array.isArray(tiposTransporte) && tiposTransporte.map((tipo) => (<MenuItem key={tipo.id} value={tipo.id}>{tipo.nombre}</MenuItem>))}
            </Select>
          </FormControl>
          <FormControl fullWidth margin="normal">
            <Select value={editingTransporte} onChange={(e) => setEditingTransporte(e.target.value)} displayEmpty>
              <MenuItem value="">Seleccionar Transporte</MenuItem>
              {Array.isArray(transportes) && transportes.map((transporte) => (<MenuItem key={transporte.id} value={transporte.id}>{transporte.nombre}</MenuItem>))}
            </Select>
          </FormControl>
