...
export default function TiposTransporte() {
  ...
  const handleSubmit = async () => { if (editRow) { await API.put(`/tipos-transporte/${editRow.id}`, form); } else { await API.post('/tipos-transporte', form); } setOpen(false); fetchData(); };
  const handleDelete = async id => { if (window.confirm('¿Borrar tipo de transporte?')) { await API.delete(`/tipos-transporte/${id}`); fetchData(); } };

  return (
    <Paper sx={{ p: 2 }}>
      <Button variant="contained" startIcon={<Add />} onClick={() => handleOpen()} sx={{ mb: 2 }}>Nuevo Tipo</Button>
