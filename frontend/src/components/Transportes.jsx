...
export default function Transportes() {
  ...
  const handleSubmit = async () => { if (editRow) { await API.put(`/transportes/${editRow.id}`, form); } else { await API.post('/transportes', form); } setOpen(false); fetchData(); };
  const handleDelete = async id => { if (window.confirm('¿Borrar transporte?')) { await API.delete(`/transportes/${id}`); fetchData(); } };

  return (
    <Paper sx={{ p: 2 }}>
      <Button variant="contained" startIcon={<Add />} onClick={() => handleOpen()} sx={{ mb: 2 }}>Nuevo Transporte</Button>
      ...
      <Dialog open={open} onClose={handleClose} maxWidth="sm" fullWidth>
        <DialogTitle sx={{ fontWeight: 700, fontSize: 22, mb: 1 }}>{editRow ? 'Editar Transporte' : 'Nuevo Transporte'}</DialogTitle>
        <DialogContent
          sx={{
            display: 'grid',
