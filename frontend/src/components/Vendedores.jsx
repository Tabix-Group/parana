...
export default function Vendedores() {
  ...
  const handleDelete = async id => { if (window.confirm('¿Borrar vendedor?')) { await API.delete(`/vendedores/${id}`); fetchData(); } };

  return (
    <Paper sx={{ p: 2 }}>
      <Button variant="contained" startIcon={<Add />} onClick={() => handleOpen()} sx={{ mb: 2 }}>Nuevo Vendedor</Button>
      ...
      <Dialog open={open} onClose={handleClose} maxWidth="sm" fullWidth>
        <DialogTitle sx={{ fontWeight: 700, fontSize: 22, mb: 1 }}>{editRow ? 'Editar Vendedor' : 'Nuevo Vendedor'}</DialogTitle>
        <DialogContent
          sx={{
            display: 'grid',
